...
def read_photometric_data(filepath):
    ...
    Format: JD, Magnitude1, Magnitude2 (positional format)
    Supports both European (comma) and American (dot) decimal separators.
    Handles missing values based on column position.
    ...
    tuple
        ...
        mag1/mag2 will have NaN where values are missing
    """
    jd = []
    mag1 = []
    mag2 = []
    ...
    try:
        ...
        for i, line in enumerate(lines):
            # Skip lines that look like headers (contain 'jd' or 'mag')
            if 'jd' in line.lower() or 'mag' in line.lower():
                start_idx = i + 1
                continue
        ...
        for line in lines[start_idx:]:
            ...
            # Find all numbers in the line with their positions
            # Use regex to find numbers (with comma or dot as decimal separator)
            import re
            # Pattern to match numbers like "14,06" or "14.06" or "13,39483036"
            pattern = r'(\d+[,.]?\d*)'
            ...
            try:
                # First match is always JD
                jd_str = matches[0].group().replace(',', '.')
                jd_val = float(jd_str)
                
                # Check positions of subsequent matches to determine mag1 vs mag2
                # Based on file format: mag1 is around position 20-30, mag2 is around position 30+
                ...
                for match in matches[1:]:
                    ...
                    # If position is < 28, it's likely mag1; otherwise mag2
                    # Also check: if there's only one value after JD and it's far right, it's mag2
                    if len(matches) == 2:
                        # Only one magnitude value
                        if pos > 25:  # Far right = mag2 (column 3)
                            mag2_val = val
                        else:  # Normal position = mag1 (column 2)
                            mag1_val = val
                    else:
                        # Two magnitude values
                        if pos < 28:
                            mag1_val = val
                        else:
                            mag2_val = val
...
def plot_lightcurve(jd, mag1, mag2, filter_name, crossings, output_file='OJ287_lightcurve.png'):
    ...
    ax.scatter(jd_sorted[valid_mag2], mag2_sorted[valid_mag2], c='#fc0303', s=30, alpha=0.9, 
